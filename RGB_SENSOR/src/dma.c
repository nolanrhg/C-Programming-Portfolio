...
void dma_i2c_mem2tx_init(void)
{
        ...
        DMA1_CSELR->CSELR |= DMA1_CH6_MAP_ON_I2C1_TX;  // DMA map to I2C1_TXDR
}
...
void dma_mem2tim1_init(uint16_t *restrict dma_mem_ptr)
{
	// Clock DMA controller 1       
        ...
        DMA1_Channel2->CPAR = (uint32_t) &(TIM1->CCR1); // data Xfer TO TIM1_CCR1 preload reg.
        DMA1_Channel2->CMAR = (uint32_t) dma_mem_ptr;  // data Xfer FROM memory region that starts at this addr. 
